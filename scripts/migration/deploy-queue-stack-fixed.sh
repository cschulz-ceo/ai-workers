#!/bin/bash
# deploy-queue-stack.sh - Deploy n8n queue mode infrastructure (Fixed for Docker Compose v5)
# Usage: bash scripts/migration/deploy-queue-stack-fixed.sh
...
check_prerequisites() {
    ...
    # Check Docker Compose (v5 uses 'docker compose')
...
wait_for_services() {
    ...
    if [ "$n8n_ready" = false ]; then
        ...
        docker logs queue-n8n-1 --tail 20
        exit 1
    fi
}
...
show_service_status() {
    ...
    echo ""
    log "Resource usage:"
    docker stats --no-stream n8n-redis n8n-postgres queue-n8n-1 2>/dev/null || true
...
# Main execution
main() {
    header "n8n Queue Mode Deployment (Fixed for Docker Compose v5)"
    ...
    if verify_deployment; then
        ...
        echo ""
        log "Next steps:"
        echo "1. Test setup: bash scripts/test/test-queue-integration.sh"
